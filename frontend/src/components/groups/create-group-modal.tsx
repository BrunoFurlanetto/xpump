...
export function CreateGroupModal({ isOpen, onClose, onCreateGroup, isLoading = false }: CreateGroupModalProps) {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            ...
            <Input
              ...
              className={`bg-background border-border text-foreground placeholder:text-muted-foreground ${errors.name ? 'border-red-500' : ''}`}
              disabled={isLoading}
              maxLength={50}
            />
            {errors.name && (
          ...
          <div className="space-y-2">
            <Label htmlFor="group-description" className="text-foreground">Descrição (opcional)</Label>
            <Textarea
              id="group-description"
              placeholder="Conte um pouco sobre o grupo..."
              value={formData.description}
              onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => handleInputChange('description', e.target.value)}
              className={`bg-background border-border text-foreground placeholder:text-muted-foreground ${errors.description ? 'border-red-500' : ''}`}
              disabled={isLoading}
              maxLength={200}
              rows={3}
            />
