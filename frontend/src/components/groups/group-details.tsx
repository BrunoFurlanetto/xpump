...
export function GroupDetails({ group, onBack }: GroupDetailsProps) {
  ...
  const handleShareGroup = async () => {
    const shareText = `Venha treinar comigo no grupo "${group.name}" no XPump! Use o código: ${group.invite_code}`;
    
    if (navigator.share) {
      try {
        await navigator.share({
          title: `Grupo ${group.name} - XPump`,
          text: shareText,
  ...
  const getPositionIcon = (position: number) => {
    switch (position) {
      ...
      default:
        return <div className="w-5 h-5 rounded-full bg-muted flex items-center justify-center text-xs font-bold">{position}</div>;
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row items-start gap-4">
        <Button onClick={onBack} variant="outline" size="sm" className="border-border text-foreground hover:bg-muted">
        ...
        <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
          <Button onClick={handleShareGroup} variant="outline" size="sm" className="border-border text-foreground hover:bg-muted">
            <Share2 className="h-4 w-4 mr-2" />
            Compartilhar
          </Button>
          <Button onClick={handleCopyInviteCode} variant="outline" size="sm" className="border-border text-foreground hover:bg-muted">
            <Copy className="h-4 w-4 mr-2" />
            <span className="hidden sm:inline">Código:</span> {group.invite_code}
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="bg-card border-border">
          <CardHeader>
            <CardTitle className="text-lg text-foreground">Classificação - {periodLabels[selectedPeriod]}</CardTitle>
          </CardHeader>
          <CardContent>
