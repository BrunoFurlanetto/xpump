...
export default function StatsPage() {
  ...
  const workoutStreak = workoutData?.streak || null;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm sm:text-base">Acompanhe sua evolução e performance detalhada</p>
        </div>
            ...
            <Card key={index} className="bg-card border-border hover:border-primary/20 transition-colors">
              <CardContent className="p-4 sm:p-6">
                ...
                <div>
                  ...
                  {stat.change && (
                    <p className={`text-xs ${getTrendColor(stat.trend)}`}>{stat.change} vs mês anterior</p>
                  )}
                </div>
              </CardContent>
            </Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              <div className="flex justify-between text-sm mb-2">
                ...
                <span className="text-foreground font-medium">{Math.round((totalPoints / 2000) * 100)}%</span>
              </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="space-y-2 text-sm text-muted-foreground">
                <p>• Sua consistência de treinos aumentou 15% comparado ao mês passado</p>
                <p>
                  • Você está próximo de bater seu recorde de sequência de {workoutStreak?.longest_streak || 0} dias
                </p>
                <p>
                  • Taxa de refeições completas está {completionRate > 75 ? "excelente" : "melhorando"} - continue
                  assim!
                </p>
                <p>• Você subiu 3 posições no ranking global este mês</p>
              </div>
            </div>
          </div>
        </CardContent>
